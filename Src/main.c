...
int main(void)
{
  ...
  // SET Custom settings
  rtP_Left.cf_nKp = SPEED_PID_P;
  rtP_Right.cf_nKp = SPEED_PID_P;
  rtP_Left.cf_nKi = SPEED_PID_I;
  rtP_Right.cf_nKi = SPEED_PID_I;
#ifdef MULTI_MODE_DRIVE
  ...
  while ((adc_buffer.l_rx2 + adc_buffer.l_tx2) >= (input1[0].min + input2[0].min))
  {
    HAL_Delay(10);
  }
#endif
